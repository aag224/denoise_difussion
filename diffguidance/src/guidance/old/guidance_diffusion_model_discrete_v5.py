...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        self.cfg = cfg
        self.args = cfg  # FIX: varios métodos (configure_optimizers, test_step, cond_sample_metric)
                         # usan self.args en vez de self.cfg; antes no existía y hubiera lanzado AttributeError.
        ...
        self.save_hyperparameters(ignore=['train_metrics', 'sampling_metrics'])  # FIX: ignore espera nombres (str), no los objetos
    ...
    def cond_fn(self, noisy_data, node_mask, target=None):
        ...
        with torch.enable_grad():
            ...
            # 4. Traducir el grafo a un vector de 2049 dims usando GraphToFP, PREENTRENADO
            # sobre fingerprints reales (ver train_graph_to_fp.py). Sustituye a la proyección
            # lineal aleatoria original: esa capa nunca se entrenaba, así que el gradiente que
            # producía no tenía relación con la química real. GraphToFP sí se entrena para
            # aproximar el fingerprint de Morgan real de cada molécula, y es invariante al
            # número de nodos (pooling global), así que sirve para cualquier tamaño de batch.
            if not hasattr(self, 'graph_to_fp_proj'):
                ...
                for p in self.graph_to_fp_proj.parameters():
                    p.requires_grad = False  # congelada: solo queremos gradiente hacia x_in, e_in
            ...
            # 6. Cálculo del error cuadrático medio (MSE) respecto al target de propiedad
            mse = torch.nn.functional.mse_loss(pred, target)

            # 6b. Término explícito de "fragmentos importantes", con SIGNO (ver
            # analyze_fragment_importance.py, que usa Integrated Gradients como en tu
            # daugfinger_randomIII.py). fragment_importance[i] < 0 significa que ese bit
            # REDUCE la ΔE_ST predicha (favorece TADF); > 0 significa que la AUMENTA
            # (la desfavorece). Minimizar sum(importance_i * bit_i) ya empuja hacia arriba
            # los bits favorables (su producto se vuelve más negativo al subir el bit) y
            # hacia abajo los desfavorables (su producto crece si el bit sube) — SIN negar
            # el término, a diferencia de una versión anterior que usaba importancia sin signo.
            ...
            if not hasattr(self, 'fragment_importance'):
                ...
                if os.path.isfile(frag_ckpt_path):
                    ...
                    self.fragment_importance = None  # no disponible: se sigue guiando solo por MSE
            if self.fragment_importance is not None:
                fp_bits = projected_input[:, :2048]  # excluye el feature de solvente
            ...
            # 7. Calcule el gradiente de la pérdida total (MSE + fragmentos) respecto a x_in y e_in
    ...
    def extract_fingerprints(self, X_single, E_single, n_bits=2048, radius=2):
        ...
        mol = self.dataset_info.to_molecule(X_single, E_single)  # FIX: era 'to_molecules' (no existe en TADFDatasetInfos)
        if mol is None:
            return None
...
def setup_tadf_components(cfg):
    ...
    dataset_infos = TADFDatasetInfos("data/tadf_dataset_fixed.pt")
    ...
    # FIX: antes se volvía a leer "data/tadf_dataset.pt" del disco por segunda vez;
    # TADFDatasetInfos ya guarda la lista cargada en self.dataset, la reutilizamos.
...
if __name__ == "__main__":
    ...
    guidance_mlp_path = os.environ.get("TADF_MLP_PATH", "C:\\Users\\mafo_\\Desktop\\mk_predictions\\tadf_mlp_model.pt")
    if not os.path.isfile(guidance_mlp_path):
        raise FileNotFoundError(
            f"No se encontró el modelo de guiado en '{guidance_mlp_path}'. "
            f"Define TADF_MLP_PATH si está en otra ubicación."
        )
    guidance_mlp = TADF_MLP(input_dim=2049, hidden_dim1=256, hidden_dim2=64).to(device)
    ...
    # FIX: la ruta absoluta de Windows solo existe en tu máquina local. La hacemos
    # configurable vía variable de entorno para que el script sea portable.
    ...
    if not os.path.isfile(guacamol_ckpt_path):
        raise FileNotFoundError(
            f"No se encontró el checkpoint en '{guacamol_ckpt_path}'. "
            f"Define la variable de entorno GUACAMOL_CKPT_PATH apuntando al .ckpt correcto."
        )
    ...
    # FIX: esto estaba indentado dentro del for, así que se ejecutaba en cada iteración
    # en vez de una sola vez al terminar de construir filtered_state_dict.
    ...
    # FIX: en vez de fijar las dimensiones de entrada a mano (18, 13), las calculamos
    # corriendo compute_extra_data sobre un grafo de prueba. Esos números dependían de
    # num_atom_types (cambia si el dataset cambia, como acaba de pasar de 11 a 13 al
    # separar Se/Si/I) y de cuántos canales agregan extra_features/domain_features, que
    # no controlamos directamente. Calcularlo en runtime evita tener que reajustar esto
    # a mano cada vez que el dataset cambie.
    # NOTA: usamos model.dataset_info (así lo guarda la clase en __init__), NO la variable
    # suelta 'dataset_infos' — esa nunca existió en este scope.
    # FIX 2: X_t/E_t deben ser ONE-HOT categórico válido, no floats aleatorios sueltos.
    # FIX 3: el grafo dummy debe ser DISPERSO, no un grafo completo. k_cycles calcula
    # ciclos con potencias de la matriz de adyacencia hasta A^6; en un grafo completo
    # (todos los nodos conectados entre sí) esos términos se vuelven grandes y las
    # cancelaciones en la fórmula de k6_cycle pueden dar un residuo negativo por error de
    # redondeo, disparando el assert (k6y >= -0.1). Un grafo disperso (aquí, una cadena
    # simple) es numéricamente estable y es más representativo de una molécula real de
    # todos modos (las moléculas no son grafos completos).
    with torch.no_grad():
        _dummy_n = 5
        ...
        for _i in range(_dummy_n - 1):
            ...
            _dummy_bond_idx[0, _i + 1, _i] = _b  # simétrico
    ...
    print(f"Dimensión de entrada detectada -> X: {true_X_in}, y: {true_y_in}")
    ...
    # FIX: antes estas dos líneas eran no-ops (asignaban una variable local a sí misma)
    # y nunca llegaban a sample_batch/cond_fn. Como resultado, cond_fn caía siempre en sus
    # valores por defecto hardcodeados (target=0.05 eV, lambda=2.5), ignorando por completo
    # lo que se configuró arriba (target_E_ST_value=0.1, guidance_scale=1.5).
    # FIX: cond_fn ahora usa GraphToFP (entrenado) en vez de una proyección aleatoria.
    # Hay que entrenarlo una vez con: python train_graph_to_fp.py --dataset data/tadf_dataset.pt
    graph_to_fp_ckpt_path = os.environ.get("GRAPH_TO_FP_CKPT", "graph_to_fp.pt")
    if not os.path.isfile(graph_to_fp_ckpt_path):
        raise FileNotFoundError(
            f"No se encontró '{graph_to_fp_ckpt_path}'. Antes de generar, entrena el puente "
            f"grafo->fingerprint con:\n"
            f"    python train_graph_to_fp.py --dataset data/tadf_dataset_fixed.pt --out {graph_to_fp_ckpt_path}"
        )
    model.graph_to_fp_ckpt_path = graph_to_fp_ckpt_path
    model.target_y = target_y

    # NUEVO: peso de fragmentos importantes (ver analyze_fragment_importance.py).
    # Si no existe el archivo, cond_fn sigue funcionando solo con el MSE de propiedad.
    ...
    if not os.path.isfile(model.fragment_importance_ckpt_path):
        print(f"⚠️ No se encontró '{model.fragment_importance_ckpt_path}'. El guiado usará solo el "
              f"MSE de propiedad, sin el término explícito de fragmentos importantes. Para activarlo:\n"
              f"    python analyze_fragment_importance.py --dataset data/tadf_dataset_fixed.pt "
    ...
    with torch.no_grad():
        ...
        generated_molecules = model.sample_batch(
            ...
            save_final=0,
            input_properties=target_y  # FIX: antes no se pasaba, así que target siempre era None
        )
        torch.save(generated_molecules, "generated_tensorsII.pt")
        print("Tensores crudos guardados en 'generated_tensorsII.pt'")

        # 2. Obtener el decodificador
        # FIX: antes esto revisaba 'dataset_infos' en locals()/globals() (nunca existe como
        # variable suelta aquí, solo como components["dataset_infos"]) y luego
        # 'model.dataset_infos' (plural — el atributo real es 'model.dataset_info', singular).
        # Como resultado ninguna rama se cumplía nunca y siempre caía a instanciar un
        # TADFDatasetInfos() nuevo desde cero (potencialmente con la ruta por defecto
        # equivocada). model.dataset_info siempre existe tras crear el modelo, así que lo
        # usamos directo.
        ...
        for i, mol in enumerate(generated_molecules):
            ...
            try:
                # Extraer átomos y enlaces
                atoms, bonds = mol[0], mol[1]

                # Asegurar dimensiones de tensores
                ...
                # Convertir a objeto Mol de RDKit usando la función to_molecule adaptada
                rdkitmol = decoder_info.to_molecule(atoms, bonds)

                if rdkitmol is not None:
                    # Intento seguro de obtener SMILES (desactivando sanitización estricta si falla)
                    try:
                        # Sanitizar explícitamente para validar la química
                        ...
                        if smiles_str and Chem.MolFromSmiles(smiles_str) is not None:
                            ...
                            status_msg = f"Molécula {i+1} [VÁLIDA]: {smiles_str}"
                        else:
                            status_msg = f"Molécula {i+1} [SMILES INVÁLIDO]: {smiles_str}"
                    except Exception as sanitize_err:
                        # Si falla la sanitización, aún extraemos el SMILES sin sanitizar para inspección
                        try:
                            smiles_str = Chem.MolToSmiles(rdkitmol, kekuleSmiles=False)
                        except Exception:
                            smiles_str = None
                        status_msg = f"Molécula {i+1} [INVÁLIDA - Error de valencia/kekulización]"
                else:
                    status_msg = f"Molécula {i+1} [INVÁLIDA - Estructura nula]"

            except Exception as e:
                status_msg = f"Molécula {i+1} [ERROR DE DECODIFICACIÓN]: {e}"
        ...
        # 3. Guardar resumen de SMILES generados en CSV
...
print(f"\n📊 Validez Química Total: {len(valid_smiles)} / {total} ({pct:.1f}%)")
print("📁 Resultados y SMILES guardados en 'generated_smiles_results.csv'\n")