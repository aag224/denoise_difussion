...
data = pd.read_csv("C:\\Users\\mafo_\\Desktop\\mk_predictions\\SolutionDataII.csv")
est = data["raw_value"]
solv = data[["eps", "ET30", "kamlet_pi"]]
...
if __name__ == "__main__":
    ...
    data_descriptors.to_excel("tadf_mordred.xlsx", index=False)

"""
tadf_mordred = pd.read_excel("C:\\Users\\mafo_\\Desktop\\mk_predictions\\tadf_mordred.xlsx")
...
solv_series = pd.DataFrame(solv)
tadf_final = pd.concat([tadf_final, solv_series], axis=1)

est_series = pd.Series(est, name="raw_value").reset_index(drop=True)
tadf_final = pd.concat([tadf_final, est_series], axis=1)
                       
tadf_final.to_excel("tadf_mordredComplete.xlsx", index=False)
